...
"""
Updated to calculate the susceptibility based on the z-axis component to allow for variance calculations
across the entire lattice, allowing for a larger sample set.
"""
...
def Metropolis(test_Lattice, test_Steps, sample_Freq, sample_Temp):
	...
	for i in range(test_Steps):
		...
		if i % sample_Freq == 0:
			...
			test_susc.append(susceptibility(test_Lattice, sample_Temp))
	...
	return test_mags, test_susc, test_Lattice
...
#	test_lattice = initiate_Lattice(lattice_Size)
	...
	susceptibility_dist = pd.DataFrame()
	t = start_Temp
	
	while t < end_Temp:
		test_samples, test_susc, test_lattice = Metropolis(test_lattice, test_Steps, sample_Freq, t)
		print(t)
		magnetic_dist[str(t)] = test_samples
		susceptibility_dist[str(t)] = test_susc
		t *= interval_Temp
		
	magnetic_dist.to_csv('{}x{}_spin_distribution_TempSweep_RandVec_ZAxis.csv'.format(lattice_Size,lattice_Size), index=False)
	susceptibility_dist.to_csv('{}x{}_susc_distribution_TempSweep_RandVec_ZAxis.csv'.format(lattice_Size,lattice_Size), index=False)
