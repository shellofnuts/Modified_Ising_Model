...
init_latticeSize = 10 # Defining the lattice size as nxn

init_warmupSteps = 20 # How many times to alter the random, initialised lattice 
max_simulatingSteps = 40 # How many times to alter the lattice at each Temperature
min_simulatingSteps = 10
sampling_Frequency = 1 # Save the lattice after ever n alters

starting_Temp = 0.8 # Temperatire to start the simulation at
stopping_Temp = 1.6 # Temperatire to finish
interval_Temp = 1.002 # Temperature step

critical_Estimate = 1.2
...
def update_TestSteps(T, Scale, deltaSteps):
	return int(max_simulatingSteps - (deltaSteps)*(abs(T-critical_Estimate)/Scale)**2)

def MCrun(lattice_Size, warmup_Steps, sample_Freq, start_Temp, end_Temp, interval_Temp):
	# Here we implement the MC simulation across multiple temperatures to find
	# the critical phenomena/phase transitions
	scale = max([abs(starting_Temp - critical_Estimate), abs(stopping_Temp-critical_Estimate)])
	delta_Steps = max_simulatingSteps - min_simulatingSteps
...
#	test_lattice = initiate_Lattice(lattice_Size)
	...
	testSteps = min_simulatingSteps
	test_list = []
	temp_list = []
	
	while t < end_Temp:
		test_samples, test_lattice = Metropolis(test_lattice, testSteps, sample_Freq, t)
		magnetic_dist.loc[:, str(t)] = pd.Series(test_samples)
		
		test_list.append(testSteps)
		temp_list.append(t)
		
		t *= interval_Temp
		testSteps = update_TestSteps(t, scale, delta_Steps)
	
	plt.plot(temp_list, test_list)
	magnetic_dist.to_csv('{}x{}_spin_distribution_VariedTempSweep.csv'.format(lattice_Size,lattice_Size), index=False)
...
MCrun(init_latticeSize, init_warmupSteps, sampling_Frequency, starting_Temp, stopping_Temp, interval_Temp)